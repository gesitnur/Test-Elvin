...
class User::DebtItemsController < User::BaseCrudController
  ...
  def index
    # @debt_items = @debt_items.where(account: params[:debt_id]).order(account_item_date: :asc)
    @debt_items = DebtItem.where(account: params[:debt_id]).page(params[:page])
  ...
  def destroy
    super do
      self.redirect_path = user_debt_items_path(debt_id: params[:debt_id])
    end
  end

  def update
    super do
      params[:debt_item][:amount] = params[:debt_item][:amount].tr(',','').to_f
      ...
      debt = Debt.find(given_params[:account_id])
      debt.calculate_balance
    end
  end
